...
const mutations = {
	...
	SET_GENERATED_DOCUMENT(state, document) {
...
const actions = {
	...
	async getTemplates({ commit }) {
		try {
			...
			commit('SET_TEMPLATES', defaultTemplates)
			...
			try {
				...
				if (response.templates && response.templates.length > 0) {
					commit('SET_TEMPLATES', response.templates)
				}
			} catch (error) {
	 ...
	 * 更新表单数据
	 */
	updateFormData({ commit }, { fieldName, value }) {
		commit('UPDATE_FORM_FIELD', { fieldName, value })
	},
	...
	setFormData({ commit }, data) {
		commit('SET_FORM_DATA', { ...state.formData, ...data })
	},
	...
	validateForm() {
		...
		state.selectedTemplate.form_config.forEach(field => {
	 ...
	 * 生成文书
	 */
	async generateDocument({ commit, dispatch }) {
		try {
			...
			// 使用本地生成
			const document = dispatch('generateDocumentLocally')
	 ...
	 * 本地生成文书
	 */
	generateDocumentLocally({ commit }) {
		...
		switch (template.id) {
			case 1: // 个人借款合同
				content = generateLoanContract(data)
				break
			case 2: // 离婚协议书
				content = generateDivorceAgreement(data)
				break
			case 3: // 劳动合同
				content = generateLaborContract(data)
				break
			case 4: // 房屋租赁合同
				content = generateRentalContract(data)
				break
			case 5: // 借条
				content = generateIOU(data)
				break
			default:
				content = generateGenericDocument(template, data)
		}
		...
		commit('SET_GENERATED_DOCUMENT', document)
	...
	async getUserDocuments({ commit }) {
		try {
			...
			commit('SET_DOCUMENTS', localDocuments)
		} catch (error) {
 ...
 * 生成借款合同
 */
function generateLoanContract(data) {
	return `个人借款合同
...
第三条 借款期限
借款期限为${data.term}个月，自${data.signing_date}起至${calculateEndDate(data.signing_date, data.term)}止。
 ...
 * 生成离婚协议书
 */
function generateDivorceAgreement(data) {
	return `离婚协议书
 ...
 * 生成劳动合同
 */
function generateLaborContract(data) {
	return `劳动合同
 ...
 * 生成租赁合同
 */
function generateRentalContract(data) {
	return `房屋租赁合同
...
第二条 租赁期限
租赁期限为${data.lease_term}个月，自${data.start_date}起至${calculateEndDate(data.start_date, data.lease_term)}止。
 ...
 * 生成通用文书
 */
function generateGenericDocument(template, data) {
 ...
 * 保存文书到本地
 */
function saveDocumentToLocal(document) {
	const localDocuments = uni.getStorageSync('userDocuments') || []
